...
class Banner {
     ...
     *
     * Intentionally tighter than \ShopEngine\Utils\Helper::get_kses_array():
     * no iframe, no form elements, no data-* passthrough. Everything here
     * arrives from a remote endpoint, so it is untrusted input -- if that
     * endpoint is ever compromised the worst it can produce is broken markup,
     * never script execution. Event handler attributes (on*) are dropped
     * automatically because wp_kses() strips every attribute not listed.
     */
